...
\brief
  Contains function definitions of funcitons from class List:
    ...
    - RemoveDuplicates - Removes duplicates in a list
*/
/*****************************************************************************/
...
namespace CS170
{
  namespace ListLab
  {
    ...
    */
    ...
    {
        // if left hand list doesnt exist return right hand list
      if(!head_)
      {
        head_ = rhs.head_;
        rhs.head_ = NULL;
        ...
        // if right hand list doesnt exist just return since theres nothing
        // to do
      if(!rhs.head_)
        return;
      
        // pointer for the left hand list
      Node *Left = head_;
      
        // pointer for right hand list
      Node *Right = rhs.head_;
      
        // pointer for current node we're at
      Node *current;
      
        // saves location for the new head after sorting
      Node *NewHead;
      
        // determine where the new head will be after the sort
      ...
      else
      {
        ...
        // while both pointer are pointing at nodes
      while(Left && Right)
      {
          // add left hand node to the list is left < right
        if(Left->number <= Right->number)
        {
          current->next = Left;
          current = current->next;
          ...
          // else add right hand node to the list
        else
        {
          current->next = Right;
          current = current->next;
        ...
        // if left is shorter than right concat right at the end
      if(!Left && Right)
        current->next = Right;
    
        // if right is shorther than left concat left at the end 
      if(Left && !Right)
        current->next = Left;
      
        //move head pointer to correct begining on new list
      head_ = NewHead;
        
        // make right hand head null 
      rhs.head_ = NULL;
    ...
    */
    ...
    int List::RemoveDuplicates()
    {
      ...
      while(walker)
      {
          // if the next node exists and the number is the same
        while(walker->next && walker->next->number == walker->number)
        {
            // pointer to skip over a node
